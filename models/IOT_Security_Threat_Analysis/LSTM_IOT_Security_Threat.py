...
def run_model(model=None, data=None):
    ...
    data_b = pd.read_csv('/Users/akram/AKRAM_CODE_FOLDER/IOT_DL/HAR/Data/cpu-utilisation.csv', date_parser=custom_date_parser,infer_datetime_format=True)
    data_b.info()
    print(f"Input DataFrame Header is =>{data_b.head()}")
    print(data_b.index)
    ...
    X_train, y_train, X_test, y_test = prepare_data(data_b, 0, 200, 100, 300)
...
# Call the Main driver  run function
